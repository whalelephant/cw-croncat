...
$BINARY tx wasm execute $CONTRACT_ADDRESS "$BANK" --amount 250000"$STAKE_TOKEN" --from user $TXFLAG -y
...
PROXY_CALL_COUNT=0
while [ "$PROXY_CALL_COUNT" -lt 3 ]; do
  ...
  echo "\033[0;35m Number of proxy calls: $PROXY_CALL_COUNT \033[0m"
